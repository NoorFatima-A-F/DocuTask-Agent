# Canary Deployment & Bad Release Rollback Runbook

**Runbook ID**: RB-006
**Target Component**: Kubernetes Deployment & Helm Releases
**Automated CLI Command**: `agy runbook exec deployment_failure --rollback-to-stable`
**Review Cadence**: Monthly Verified

---

## 1. Detection
- **Prometheus Alert**: `Alert: KubernetesDeployment&HelmReleasesDegraded`
...
## 2. Impact
- **Service Affected**: Kubernetes Deployment & Helm Releases
...
1. Check component status:
   ```bash
   kubectl get pods -l app=deployment -n docutask
   kubectl logs --tail=100 -l app=deployment -n docutask
   ```
...
```bash
agy runbook exec deployment_failure --rollback-to-stable
```
...
2. Trigger standby promotion / pod restart:
   ```bash
   kubectl rollout restart deployment/deployment -n docutask
   ```
...
- Run health validation probe:
  ```bash
  python -m pytest tests/platform_verification/test_enterprise_operational_resilience.py -k "deployment"
  ```
...
```bash
agy runbook rollback --runbook-id RB-006 --restore-snapshot
```
