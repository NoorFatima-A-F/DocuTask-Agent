...
def main() -> int:
    ...
    print("=" * 95)
    print(f"  Configuration Sources       : {inv.configuration_sources} ({inv.environment_variables} Env Vars, {inv.config_files} Files, {inv.external_secret_stores} External Stores)")
    print(f"  Configuration Catalog Items : {cat.total_catalog_items} Parameters across {len(cat.categories_covered)} Categories")
    print(f"  Discovered Secrets Audited  : {sec_inv.total_secrets_discovered} ({sec_inv.plaintext_exposures_found} Plaintext Exposures)")
    ...
    print(f"  [4] Secret Discovery & Scan : {sec_inv.total_secrets_discovered} Secrets Scanned (Gitleaks, TruffleHog, Detect-Secrets) [PASSED]")
