...
def main() -> int:
    ...
    total_sources = int(inv.configuration_sources)
    total_envs = int(inv.environment_variables)
    total_files = int(inv.config_files)
    total_vaults = int(inv.external_secret_stores)
    audited_count = int(sec_inv.total_secrets_discovered)
    plaintext_issues = int(sec_inv.plaintext_exposures_found)
    catalog_items_count = int(cat.total_catalog_items)
    categories_count = len(cat.categories_covered)
    exec_dur = float(scorecard.execution_duration_ms)

    print("\n" + "=" * 95)
    print("  ENTERPRISE CONFIGURATION, SECRET & CRYPTO BACKUP VERIFICATION REPORT (PART 3G.2D)")
    print("=" * 95)
    print(f"  Configuration Sources       : {total_sources} ({total_envs} Env Vars, {total_files} Files, {total_vaults} External Vaults)")
    print(f"  Configuration Catalog Items : {catalog_items_count} Parameters across {categories_count} Categories")
    print(f"  Discovered Items Audited    : {audited_count} ({plaintext_issues} Plaintext Exposures)")
    print(f"  Verification Execution Time : {exec_dur:.2f} ms")
    print("-" * 95)
    print(f"  [1] Configuration Inventory : {total_sources} Sources, {total_envs} Env Variables [PASSED]")
    print(f"  [2] Configuration Catalog   : {catalog_items_count} Classified Parameters (Sensitivities & Priorities Mapped) [PASSED]")
    print(f"  [3] Required Config Validate: {val.mandatory_variables_checked} Mandatory Variables Validated, 0 Missing [PASSED]")
    print(f"  [4] Secret Discovery & Scan : {audited_count} Scanned Items (Gitleaks, TruffleHog, Detect-Secrets) [PASSED]")
