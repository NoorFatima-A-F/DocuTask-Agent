...
interface MissionControlContextValue {
  ...
  pauseMission: (reason?: string) => void;
  resumeMission: (reason?: string) => void;
  approveDecision: (decisionId: string) => void;
  rejectDecision: (decisionId: string, reason: string) => void;
  overrideDecision: (decisionId: string, newAction: string, reason: string) => void;
  abortMission: (reason: string) => void;
  requestExplanation: (decisionId: string) => void;
  selectDagNode: (nodeId?: string) => void;
  selectDecision: (decisionId?: string) => void;
  toggleLiveSimulation: () => void;
  injectSentinelState: () => void;
...
export const MissionControlProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [state, setState] = useState<MissionControlState>(initialMissionControlState);

  // Periodic clock and simulated thought stream ticker
  useEffect(() => {
    if (!state.isSimulatingLive || state.isPaused) return;
    ...
    const timer = setInterval(() => {
      setState((prev) => {
        ...
        if (newElapsed % 8 === 0) {
          const agents: ('PLANNER' | 'EVIDENCE' | 'MEMORY' | 'STATISTICS' | 'GOVERNANCE' | 'COORDINATOR' | 'REFLECTION')[] = [
            'STATISTICS',
            'REFLECTION',
            'EVIDENCE',
            'MEMORY',
          ...
          const newMsg: AgentThoughtMessage = {
            id: `msg_${Date.now()}`,
  ...
  const pauseMission = useCallback((reason = 'User requested pause') => {
    setState((prev) => {
      const audit: HumanControlAuditEntry = {
        id: `audit_${Date.now()}`,
        timestampUtc: new Date().toISOString().substring(11, 19),
        action: 'PAUSE',
  ...
  const approveDecision = useCallback((decisionId: string) => {
    setState((prev) => {
      const audit: HumanControlAuditEntry = {
        id: `audit_${Date.now()}`,
        timestampUtc: new Date().toISOString().substring(11, 19),
  ...
  const overrideDecision = useCallback((decisionId: string, newAction: string, reason: string) => {
    setState((prev) => {
  ...
  const abortMission = useCallback((reason: string) => {
    setState((prev) => {
      const audit: HumanControlAuditEntry = {
        id: `audit_${Date.now()}`,
        timestampUtc: new Date().toISOString().substring(11, 19),
  ...
  const requestExplanation = useCallback((decisionId: string) => {
    setState((prev) => {
      const audit: HumanControlAuditEntry = {
        id: `audit_${Date.now()}`,
        timestampUtc: new Date().toISOString().substring(11, 19),
  ...
  const selectDagNode = useCallback((nodeId?: string) => {
