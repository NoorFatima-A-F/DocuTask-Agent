/**
 * Phase 13.21 Knowledge API Client
 */
import {
  KnowledgeAsset,
  KnowledgeSource,
...
export const knowledgeApiClient = {
  async search(query: string, topK = 5, tenantId = 'default-tenant'): Promise<RetrievedSnippet[]> {
    try {
  ...
  async retrieveContext(goal: string, tenantId = 'default-tenant'): Promise<ContextRetrievalResponse> {
    try {
      ...
      if (!res.ok) throw new Error(`Retrieve context failed: ${res.statusText}`);
      return await res.json();
    } catch {
      return {
        query: goal,
  ...
  async listAssets(tenantId = 'default-tenant'): Promise<KnowledgeAsset[]> {
    try {
  ...
  async ingestAsset(payload: {
    ...
    if (!res.ok) throw new Error(`Ingest asset failed: ${res.statusText}`);
    return await res.json();
  },

  async listSources(tenantId = 'default-tenant'): Promise<KnowledgeSource[]> {
    try {
      const res = await fetch(`${BASE_URL}/sources?tenant_id=${encodeURIComponent(tenantId)}`);
      if (!res.ok) throw new Error(`List sources failed: ${res.statusText}`);
      return await res.json();
    } catch {
      return [
        {
  ...
  async getGraphOverview(tenantId = 'default-tenant'): Promise<{
    total_nodes: number;
    total_edges: number;
    nodes: GraphNode[];
    edges: GraphEdge[];
  ...
  async listMemories(tenantId = 'default-tenant'): Promise<MemoryEntry[]> {
    try {
      const res = await fetch(`${BASE_URL}/memory/list?tenant_id=${encodeURIComponent(tenantId)}`);
      if (!res.ok) throw new Error(`List memories failed: ${res.statusText}`);
      return await res.json();
    } catch {
      return [
        {
  ...
  async getQualityReport(tenantId = 'default-tenant'): Promise<KnowledgeQualityReport> {
    try {
      const res = await fetch(`${BASE_URL}/quality/report?tenant_id=${encodeURIComponent(tenantId)}`);
      if (!res.ok) throw new Error(`Quality report failed: ${res.statusText}`);
      return await res.json();
    } catch {
      return {
        tenant_id: tenantId,
  ...
  async runOptimization(tenantId = 'default-tenant'): Promise<any> {
    const res = await fetch(`${BASE_URL}/optimize?tenant_id=${encodeURIComponent(tenantId)}`, { method: 'POST' });
    return await res.json();
  },

  async runEvaluation(tenantId = 'default-tenant'): Promise<any> {
