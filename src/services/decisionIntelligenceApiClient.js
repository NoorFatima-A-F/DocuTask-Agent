...
export class DecisionIntelligenceApiClient {
    ...
    static async getBeliefs() {
        try {
            return await this.request('/beliefs');
        }
        catch {
            return this.getFallbackBeliefs();
        }
    }
    static async updateBayesianObservation(payload) {
        try {
            return await this.request('/bayesian/update', {
                method: 'POST',
    ...
    static async getWorldForecast() {
        try {
            return await this.request('/world/forecast');
        }
        catch {
            return {
                horizons: {
    ...
    static async evaluateEVOI(actions) {
        try {
            return await this.request('/evoi/evaluate', {
                method: 'POST',
    ...
    static async getMetaCritique(missionId) {
        try {
            return await this.request(`/meta/critique?mission_id=${missionId}`);
        }
        catch {
            return {
    ...
    static async verifyGovernance(planId) {
        try {
            return await this.request('/governance/verify', {
                method: 'POST',
                body: JSON.stringify({
                    plan_id: planId,
                    cost_limit_usd: 10.0,
    ...
    static async getDecisionProvenance(planId) {
        try {
            return await this.request(`/governance/provenance/${planId}`);
        }
        catch {
            return [
                {
    ...
    static async runBenchmarkSuite() {
        try {
            return await this.request('/benchmark/run', {
                method: 'POST',
            });
        }
        catch {
