...
} from '../types/autonomousPlanning';

const API_BASE = '/api/v1/planning';

export class PlanningApiClient {
  static async planMission(missionId: string, intent: string, userConstraints?: Record<string, any>): Promise<MissionPlanResult> {
    try {
  ...
  static async getStrategyMatrix(missionId: string): Promise<StrategyComparisonMatrix> {
    try {
  ...
  static async queryCounterfactual(
    missionId: string,
    queryType: string,
  ...
  static async getDAG(missionId: string): Promise<MutableExecutionDAG> {
    try {
  ...
  static async mutateDAG(missionId: string, mutationType: string, targetNodeId: string, splitCount: number = 2): Promise<MutableExecutionDAG> {
    try {
  ...
  static async getSchedulerStatus(): Promise<SchedulerClusterStatus> {
    try {
  ...
  static async evaluateMission(missionId: string, actualLatencyMs: number, actualCostUsd: number, actualAccuracy: number): Promise<PlanCalibrationMetric> {
    try {
  ...
  private static getFallbackPlan(missionId: string, intent: string): MissionPlanResult {
    return {
