...
import type {
  ...
  LifecycleOverview,
  AgentCategory,
} from '../types/aiLifecycle';

const BASE_URL = '/api/v1/ai-lifecycle';

export class AILifecycleApiClient {
  static async getOverview(): Promise<LifecycleOverview> {
    try {
  ...
  static async listAgents(): Promise<AgentApplication[]> {
    try {
  ...
  static async registerAgent(payload: any): Promise<AgentApplication> {
    const res = await fetch(`${BASE_URL}/agents`, {
      method: 'POST',
  ...
  static async listVersions(agentId: string): Promise<AgentVersion[]> {
    try {
  ...
  static async rollbackVersion(agentId: string, versionTag: string): Promise<AgentVersion> {
  ...
  static async runTests(agentId: string, versionTag = '1.0.0'): Promise<AgentTestResult> {
    try {
  ...
  static async scanSecurity(agentId: string, payload: any): Promise<AgentSecurityScan> {
    try {
  ...
  static async listApprovals(agentId: string): Promise<AgentApproval[]> {
    try {
  ...
  static async deployAgent(agentId: string, payload: any): Promise<AgentDeployment> {
  ...
  static async listDependencies(agentId: string): Promise<AgentDependency[]> {
    try {
  ...
  static async listTemplates(category?: AgentCategory): Promise<AgentTemplate[]> {
    try {
  ...
  static async listMarketplaceListings(): Promise<AgentMarketplaceListing[]> {
    try {
  ...
  static async retireAgent(agentId: string, payload: any): Promise<AgentRetirementPlan> {
