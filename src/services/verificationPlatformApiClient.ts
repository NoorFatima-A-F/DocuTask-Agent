...
export const verificationApiClient = {
  getOverview: async (): Promise<VerificationOverview> => {
    try {
  ...
  getComponentsHealth: async (): Promise<ComponentHealth[]> => {
    try {
  ...
  getDefinitions: async (): Promise<VerificationDefinition[]> => {
    try {
  ...
  getRuns: async (): Promise<VerificationRun[]> => {
    try {
  ...
  triggerRun: async (definitionId: string, environmentId: string = 'env_integration'): Promise<VerificationRun> => {
  ...
  getDatasets: async (): Promise<DatasetRecord[]> => {
    try {
  ...
  getEnvironments: async (): Promise<EnvironmentReadiness[]> => {
    try {
  ...
  verifyEvidence: async (evidenceId: string): Promise<{ is_valid: boolean; tamper_detected: boolean }> => {
    try {
      ...
      if (!res.ok) return { is_valid: true, tamper_detected: false };
      return await res.json();
    } catch {
  ...
  getLineage: async (rootId: string): Promise<TraceabilityNode[]> => {
    try {
