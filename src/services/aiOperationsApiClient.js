...
export class AIOperationsApiClient {
    ...
    static async getFleetTelemetry() {
        try {
            return await this.request('/telemetry/agents');
        }
        catch {
            return [
                {
    ...
    static async getTraces(limit = 50, agentId) {
        try {
            const q = new URLSearchParams({ limit: String(limit) });
            if (agentId)
                q.append('agent_id', agentId);
            return await this.request(`/telemetry/traces?${q.toString()}`);
        }
        catch {
            return [
                {
    ...
    static async getEvaluationResults(limit = 50, agentId) {
        try {
            const q = new URLSearchParams({ limit: String(limit) });
            if (agentId)
                q.append('agent_id', agentId);
            return await this.request(`/evaluation/results?${q.toString()}`);
        }
        catch {
            return [
                {
    ...
    static async routeModel(params) {
        try {
            return await this.request('/optimization/model-route', {
                method: 'POST',
    ...
    static async getPrompts(agentId) {
        try {
            const q = agentId ? `?agent_id=${agentId}` : '';
            return await this.request(`/optimization/prompts${q}`);
        }
        catch {
            return [
                {
    ...
    static async getCostAnalytics() {
        try {
            return await this.request('/cost/analytics');
        }
        catch {
            return {
                total_tokens_consumed: 142500,
    ...
    static async getFailureDiagnoses(limit = 50, agentId) {
        try {
            const q = new URLSearchParams({ limit: String(limit) });
            if (agentId)
                q.append('agent_id', agentId);
            return await this.request(`/debugging/failures?${q.toString()}`);
        }
        catch {
            return [
                {
    ...
    static async getPredictiveRisks(limit = 20) {
        try {
            return await this.request(`/prediction/risks?limit=${limit}`);
        }
        catch {
            return [
                {
    ...
    static async getProposals(agentId) {
        try {
            const q = agentId ? `?agent_id=${agentId}` : '';
            return await this.request(`/improvement/proposals${q}`);
        }
        catch {
            return [
                {
    ...
    static async approveProposal(proposalId, actor = 'Enterprise Administrator') {
        return await this.request(`/improvement/proposals/${proposalId}/approve`, {
            method: 'POST',
            body: JSON.stringify({ actor, reason: 'Approved by human reviewer' }),
        });
    }
    static async rejectProposal(proposalId, actor = 'Enterprise Administrator', reason = 'Rejected') {
        return await this.request(`/improvement/proposals/${proposalId}/reject`, {
            method: 'POST',
    ...
    static async getExperiments() {
        try {
            return await this.request('/experiments');
        }
        catch {
            return [
                {
    ...
    static async runExperiment(params) {
        return await this.request('/experiments/run', {
            method: 'POST',
            body: JSON.stringify(params),
        });
    }
    static async getGovernanceAuditLogs(limit = 50, agentId) {
        try {
            const q = new URLSearchParams({ limit: String(limit) });
            if (agentId)
                q.append('agent_id', agentId);
            return await this.request(`/governance/audit-logs?${q.toString()}`);
        }
        catch {
            return [
                {
    ...
    static async runOperationsCycle(agentId = 'agent_chief_architect') {
        return await this.request('/runtime/cycle', {
            method: 'POST',
