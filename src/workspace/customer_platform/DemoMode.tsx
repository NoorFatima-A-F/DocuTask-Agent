  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 rounded-2xl bg-gradient-to-r from-[#0F172A] to-[#1E293B] border border-[#334155]/60 shadow-xl">
        <div>
          <Badge variant="intelligence" size="sm">1-CLICK LIVE DEMONSTRATION</Badge>
          <h1 className="text-2xl font-black text-white mt-1">Interactive Enterprise Automation Player</h1>
          <p className="text-sm text-[#94A3B8]">
            Simulate an end-to-end multi-agent document lifecycle in real-time.
          </p>
        </div>
        <Button
          variant="primary"
          ...
          disabled={isRunning}
          className="bg-gradient-to-r from-[#0066FF] to-[#00D2FF] text-white shadow-lg shadow-[#00D2FF]/20"
        >
          {isRunning ? 'Processing Multi-Agent Flow...' : '▶ Run Invoice Automation Demo'}
        </Button>
      </div>
            ...
            <div
              ...
              className={`p-5 rounded-2xl border transition-all space-y-2.5 ${
                isCurrent
                  ...
                  : 'bg-[#0A0F1D]/60 border-[#1E293B] opacity-50'
              }`}
            >
              <div className="flex items-center justify-between">
                <Badge variant="default" size="sm">{st.agent}</Badge>
                {isPassed && <span className="text-emerald-400 text-xs font-bold">✓ DONE</span>}
                {isCurrent && <span className="text-cyan-400 text-xs font-bold animate-pulse">● RUNNING</span>}
              </div>

              <h3 className="text-xs font-bold text-white">{st.title}</h3>
              <p className="text-[11px] text-[#94A3B8] leading-relaxed">{st.desc}</p>
            </div>
          );
        })}
      </div>
      ...
      {currentStage === stages.length && (
        <div className="p-6 rounded-2xl bg-gradient-to-r from-emerald-950/40 to-[#0F172A] border border-emerald-500/50 shadow-2xl flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="space-y-1">
            <span className="text-xs font-bold text-emerald-400 font-mono">AUTOMATION COMPLETED IN 285ms</span>
            <h2 className="text-lg font-black text-white">Invoice INV-2026-8891 Cleared &amp; Posted to QuickBooks</h2>
            <p className="text-xs text-[#CBD5E1]">
              Saved $34.975 vs manual processing. 0 human touches required. 100% cryptographic audit trail generated.
            </p>
          </div>
          <Button variant="secondary" size="md" onClick={() => setCurrentStage(0)}>
            Reset Simulation
          </Button>
        </div>
      )}
