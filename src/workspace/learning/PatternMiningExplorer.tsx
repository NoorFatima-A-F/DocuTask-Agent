...
export const PatternMiningExplorer: React.FC = () => {
    ...
    <div className="p-6 space-y-6 bg-[#0B1120] min-h-screen text-[#F8FAFC]">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#1E293B] pb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold font-mono tracking-tight text-white flex items-center gap-2">
              Execution Pattern Mining Explorer
              <Badge variant="intelligence" size="sm">Phase 13.5</Badge>
            </h1>
            <p className="text-xs text-[#94A3B8] font-mono">
              Unsupervised and supervised pattern mining across mission replay logs to discover high-utility workflow wavefronts
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search mined patterns..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="bg-[#0F172A] border border-[#334155] rounded-lg pl-9 pr-3 py-1.5 text-xs font-mono text-white focus:outline-none focus:border-purple-500"
            />
          </div>
          <select
          ...
          >
            ...
            <option value="PARALLEL_WAVEFRONT">Parallel Wavefront</option>
            <option value="INVARIANT_VERIFICATION">Invariant Verification</option>
            <option value="FALLBACK_CASCADE">Fallback Cascade</option>
            <option value="CONFIDENCE_RECOVERY">Confidence Recovery</option>
          </select>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 font-mono">
        {filteredPatterns.map((p) => (
          <Card key={p.id} className="p-5 rounded-xl border border-[#1E293B] bg-[#0F172A] space-y-4 flex flex-col justify-between">
            <div className="space-y-3">
              <div className="flex items-start justify-between gap-2">
                <Badge variant="outline" size="sm">{p.type}</Badge>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-[#94A3B8]">{p.frequency} Occurrences</span>
                  <Badge variant="success" size="sm">{p.successRate}% Success</Badge>
                </div>
              </div>

              <div>
                <h3 className="text-sm font-bold text-white">{p.name}</h3>
                <p className="text-xs text-[#94A3B8] mt-1 leading-relaxed">{p.description}</p>
              </div>

              {/* Task Sequence Visualizer */}
              <div className="p-3 bg-[#0B1120] border border-[#1E293B] rounded-xl space-y-1.5">
                <span className="text-[10px] text-[#64748B] block">TASK EXECUTION WAVEFRONT:</span>
                <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
                  {p.tasks.map((task, idx) => (
                    <React.Fragment key={idx}>
                      ...
                      {idx < p.tasks.length - 1 && <ArrowRight className="w-3 h-3 text-[#64748B]" />}
                    </React.Fragment>
                  ))}
                </div>
              </div>
            ...
            <div className="pt-3 border-t border-[#1E293B] flex items-center justify-between text-xs">
              <span className="text-emerald-400 font-bold flex items-center gap-1">
                ...
                Gain: {p.throughputGain}
              </span>
              <span className="text-[11px] text-[#64748B]">Pattern ID: {p.id}</span>
            </div>
          </Card>
        ))}
      </div>
