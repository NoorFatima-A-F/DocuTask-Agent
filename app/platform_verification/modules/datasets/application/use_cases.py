...
class ManageDatasetsUseCase:
    ...
    def create(self, name: str, tenant_id: str = "default-tenant", **metadata) -> Result[DatasetsEntity, str]:
        try:
            entity = DatasetsEntity(name=name, tenant_id=tenant_id, metadata=metadata)
            saved = self._repo.save(entity)
            return Success(saved)
        except Exception as e:
            return Failure(str(e))

    def get(self, entity_id: str) -> Optional[DatasetsEntity]:
        return self._repo.get_by_id(entity_id)

    def list(self, tenant_id: str = "default-tenant") -> List[DatasetsEntity]:
